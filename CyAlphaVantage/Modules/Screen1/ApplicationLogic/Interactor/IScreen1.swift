...
class IScreen1: NSObject {
    ...
    func fetchDefaultData(strSymbol: String, completionHandler: @escaping(_ data: [ModelSymbolDetail]) -> Void) {
        ...
        self.task = URLSession.shared.dataTask(with: request, completionHandler: { (data, response, error) -> Void in
            ...
            if let data = data {
                do {
                    ...
                    if !DataManager.shared.isDaily {
                        key = "Time Series (\(DataManager.shared.getFunctionInterval()))"
                    }
    ...
    func fetchSearchData(str: String, completionHandler: @escaping(_ data: ModelSearchResult) -> Void ) {
        print("Search Data", self.configureURLSearch(str: str))
        self.task?.cancel()
      
        self.isLoading = true
        guard let url = URL(string: self.configureURLSearch(str: str)) else {
